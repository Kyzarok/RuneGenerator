...
def convert_data():
    ...
    path = 'big_data/'
    count = 0
    for pic_name in glob.glob(os.path.join(path, '*.png')):
        im = Image.open(pic_name) # Can be many different formats.
...
def load_data():
    get_data, length_of_data = convert_data()
    x_train = get_data
    ...
    # 784 columns per row
    x_train = x_train.reshape(length_of_data, 4096)
    return x_train


def adam_optimizer():
    return Adam(lr=0.000085, beta_1=0.5)
